...
import { TonToken, EIP820Registry } from '../helpers/artifacts';
...
const shouldBehaveLikeTonToken = (admin, operator, recipient, upgradeable) => {
  ...
  beforeEach(async () => {
    if (upgradeable >= 0) {
      ...
      [tonToken, initialSupply] = await upgradeToV(admin);
      const name = await tonToken.name();
      assert.equal('Upgradeable NORI Token', name);
    } else {
      eip820Registry = await EIP820Registry.deployed();
      tonToken = await deployContract(
        TonToken,
        ['Ton Token', 'TON', 1, 0, eip820Registry.address],
        { from: admin }
      );
      const name = await tonToken.name();
      assert.equal('Ton Token', name);
    }
    assert.ok(tonToken);
